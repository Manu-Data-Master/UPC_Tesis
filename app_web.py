...
def update_dashboard():
    if st.session_state.count_pose_g == 0:
        ...
        placeholder_pose_global.metric("POSE GLOBAL", str(st.session_state.count_pose_g) + " / " + str(st.session_state.total_poses), "+1 pose g.")
        ...
        placeholder_set.metric("SET", str(st.session_state.count_set) + " / "+ str(st.session_state.n_sets), "  +1 set")
        ...
        if st.session_state.count_pose_g < st.session_state.total_poses:
            ...
            placeholder_status.markdown(font_size_px("🏎️ TRAINING..."), unsafe_allow_html=True)
            placeholder_pose_global.metric("POSE GLOBAL", str(st.session_state.count_pose_g) + " / " + str(st.session_state.total_poses), "+1 pose g.")
            placeholder_pose.metric("POSE", str(st.session_state.count_pose) + " / "+ str(st.session_state.n_poses),)
            placeholder_rep.metric("REPETITION", str(st.session_state.count_rep) + " / "+ str(st.session_state.n_reps),)
            ...
            if st.session_state.count_pose < st.session_state.n_poses:
                ...
                st.session_state.count_pose = 1
                placeholder_pose.metric("POSE", str(st.session_state.count_pose) + " / "+ str(st.session_state.n_poses))
                if st.session_state.count_rep < st.session_state.n_reps:
                    st.session_state.count_rep += 1
                    placeholder_rep.metric("REPETITION", str(st.session_state.count_rep) + " / "+ str(st.session_state.n_reps), "+1 rep")
            ...
            placeholder_pose.metric("POSE", str(st.session_state.count_pose) + " / "+ str(st.session_state.n_poses), "COMPLETED", delta_color="inverse")
                        ...
                        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
                            ...
                            #while st.session_state.count_rep < st.session_state.n_reps:
                                ...
                                if results.pose_landmarks is None:
                                    ...
                                    ############################################
                                    #PRUEBA: AUMENTA LA POSE AL TAPAR LA CÁMARA
                                    ############################################
